...
export class TaskController {
  async list(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      const { data, total } = await taskService.list(req.teamId!, page, limit);
      res.json(paginatedResponse(data, total, page, limit));
    } catch (e) { next(e); }
  }
  async getById(req: AuthRequest, res: Response, next: NextFunction) {
    try { res.json(apiResponse(await taskService.getById(Number(req.params.id), req.teamId!))); } catch (e) { next(e); }
  }
  async getByDate(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      res.json(apiResponse(await taskService.getByDate(parsed, req.teamId!)));
    } catch (e) { next(e); }
  }
  ...
  async create(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      res.status(201).json(apiResponse(await taskService.create(req.body, req.user!.id, req.user!.name, req.teamId!), 'Tarefa criada'));
    } catch (e) { next(e); }
  }
  async update(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      res.json(apiResponse(await taskService.update(Number(req.params.id), req.body, req.user!.id, req.user!.name, req.teamId!), 'Tarefa atualizada'));
    } catch (e) { next(e); }
  }
  async delete(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      await taskService.delete(Number(req.params.id), req.user!.id, req.user!.name, req.teamId!);
      res.json(apiResponse(null, 'Tarefa deletada'));
    } catch (e) { next(e); }
  }
  async getPendingCount(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      res.json(apiResponse({ count: await taskService.getPendingCount(req.teamId!) }));
    } catch (e) { next(e); }
  }
}
