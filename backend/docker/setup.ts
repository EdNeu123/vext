...
function isPgReady(): boolean {
    try {
        execSync('docker exec vext-postgres pg_isready -U vext -d vext_crm', {
            stdio: 'ignore',
        });
        return true;
    } catch {
...
async function main(): Promise<void> {
    ...
    if (!isDockerRunning()) {
        console.error(
            red('❌ Docker daemon não está rodando! Inicie o Docker Desktop.')
        );
        process.exit(1);
    }
    ...
    if (!existsSync(envFile)) {
        console.log(
            yellow(
                '  ⚠ Arquivo .env não encontrado. Copiando .env.example → .env'
            )
        );
        copyFileSync(envExample, envFile);
        console.log(
            green('  ✔ .env criado — revise as variáveis se necessário')
        );
    } else {
