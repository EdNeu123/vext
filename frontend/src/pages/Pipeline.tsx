...
import { cardService, contactService } from '../services';
...
export default function Pipeline() {
  ...
  const { data: result } = useQuery({ queryKey: ['cards'], queryFn: () => cardService.list(1, 200) });
  const { data: contacts } = useQuery({ queryKey: ['contacts-list'], queryFn: () => contactService.list('', 1, 200) });
  const cards = (result?.data || []) as Card[];
  const contactList = (contacts?.data || []) as any[];

  const [showModal, setShowModal] = useState(false);
  const [dragDeal, setDragDeal] = useState<number | null>(null);
  const [form, setForm] = useState({ title: '', value: '', contactId: '', stage: 'prospecting' as CardStage });

  const createMut = useMutation({
    mutationFn: (data: any) => cardService.create(data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['cards'] }); setShowModal(false); toast.success('Card criado!'); },
  ...
  const handleDragStart = (cardId: number) => setDragDeal(cardId);
  const handleDragOver = (e: React.DragEvent) => e.preventDefault();
  const handleDrop = (stage: CardStage) => {
    if (dragDeal) {
      const card = cards.find((d) => d.id === dragDeal);
      if (card && card.stage !== stage) {
        updateMut.mutate({ id: dragDeal, data: { stage } });
        toast.success(`Card movido para ${STAGES.find((s) => s.key === stage)?.label}`);
      }
      setDragDeal(null);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">Arraste os cards entre as colunas</p>
        </div>
      ...
      <div className="flex gap-4 overflow-x-auto pb-4">
        {STAGES.map((stage) => {
          const stageDeals = cards.filter((d) => d.stage === stage.key);
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          ...
                          <span className="text-xs font-semibold text-emerald-400">{formatCurrency(Number(card.value))}</span>
                          <span className="text-[10px] text-gray-600 ml-auto">{card.probability}%</span>
                        </div>
      ...
      {/* Create Modal */}
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Novo Card">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div>
            ...
            <select value={form.stage} onChange={(e) => setForm({ ...form, stage: e.target.value as CardStage })}
              className="w-full px-4 py-2.5 bg-gray-800 border border-gray-700 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-indigo-500">
